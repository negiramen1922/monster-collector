...
| rune_test.js | ルーン: 伸び率(1Tierごとに同じ幅)、攻撃型・防御型、合成(土台+同じTier・同じタイプ2つでTier+1、レア度とサブ効果は土台のまま・値はTierで伸びる)、編成ごとの装備(同じ編成の中だけ重複不可)、ステータス・BPへの反映、分解・上限、ルーン採掘の出方、PVP防衛への反映と不正値の丸め |
| rune_ui_test.py | キャラ → ルーン の一覧・絞り込み、ルーン詳細(合成・ロック)、キャラ詳細の4枠から装備・付け替え、まとめて分解、育成クエストのルーン採掘(7段階・いっぱいだと出られない)を画面で確かめる |
| star10_test.js | ★10解放: docs/design/今後やりたいこと.json の確定表(上限Lv・必要ソウル・★1〜★6据え置き)、Lv300までのEXPと壁、敵の★上限が7のままか、★7→★10まで昇格できるか |
| news_popup_ui_test.py | 新しいニュースがあるとゲームに入ったときにお知らせ(ニュース)が開き、既読後・アップデート内容だけ新しいとき・はじめたばかりのときは出ないことを画面で確かめる |
| guide_help_ui_test.py | チュートリアルのある画面(キャラ詳細・遺物図鑑・深淵回廊・ルーン採掘)の右上の「?」で説明が何度でも開くこと、ない画面には出ないことを画面で確かめる |
| sfx_ui_test.py | 効果音がすべて合成音(GAME_SFX)で、コードで鳴らしている名前がすべて定義されていること、全部鳴らしても・戦闘をまわしてもエラーが出ないことを確かめる |
