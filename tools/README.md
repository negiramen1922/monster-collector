...
| rune_test.js | ルーン: レアリティ(最初のサブの数)とTier(数値の桁)、強化(+3ごとにサブが増える/伸びる)、編成ごとの装備(同じ編成の中だけ重複不可)、ステータス・BPへの反映、分解・上限、ルーン採掘の出方、PVP防衛への反映と不正値の丸め |
| rune_ui_test.py | キャラ → ルーン の一覧・絞り込み、ルーン詳細(強化・ロック)、キャラ詳細の4枠から装備・付け替え、まとめて分解、育成クエストのルーン採掘(6段階・いっぱいだと出られない)を画面で確かめる |
