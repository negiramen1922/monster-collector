...
const api = load('game.js', src => src + `;global.__e = {
  ...
  beginnerAllDone, buyShopItem, SHOP_GOLD_ITEMS, renderMissions, renderBeginnerCard,
...
ok('続きの b11 から始まる', entry('b:b11').state !== 'locked' && entry('b:b10').state === 'done');
ok('ガイド1のすべて達成は受け取り済みのまま', entry('b:all').state === 'done');
ok('初心者ミッションは19個', E.BEGINNER_MISSIONS.length === 19);
...
ok('b11(PVP編成)を受け取れて報酬が入る', !!got && T.crystals === crystals0 + 100 && T.beginnerStep === 11, [!!got, T.crystals - crystals0]);
...
ok('b12〜b19 を順に受け取れる', T.beginnerStep === 19, T.beginnerStep);
