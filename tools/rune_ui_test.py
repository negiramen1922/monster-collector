...
async def main():
    with game_url() as url:
        async with async_playwright() as p:
            ...
            check('ショップに「ルーンの粉ショップ」(3品)', await pg.locator('#screen [data-rune-shop-buy]').count() == 3)
            await pg.screenshot(path=str(OUT / 'rune_shop.png'))
            n0 = await pg.evaluate("() => STATE.runes.length")
            await pg.evaluate("() => document.querySelector('[data-rune-shop-buy=\"rune_gold\"]').click()"); await pg.wait_for_timeout(200)
            check('交換すると増えて、そのルーンの詳細が開く(金以上)', await pg.evaluate("() => STATE.runes.length") == n0 + 1 and await pg.evaluate("() => runeDetail && runeByUid(runeDetail.uid).rarity >= 3"))
