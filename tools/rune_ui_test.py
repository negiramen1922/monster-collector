...
async def main():
    with game_url() as url:
        async with async_playwright() as p:
            ...
            check('振り直すと粉が減り、変化が表示される', await pg.evaluate("() => STATE.runeDust") < d0 and '→' in await pg.inner_text('.rd-flash'))
            await pg.evaluate("() => document.querySelectorAll('.rr-row')[0].scrollIntoView()")
            await pg.screenshot(path=str(OUT / 'rune_reroll.png'))
            ...
            # ルーンの粉ショップ
            await pg.evaluate("() => { goto('shop', {nav:true}); document.querySelector('[data-shop-tab=\"rune\"]').click(); }"); await pg.wait_for_timeout(200)
            rows = await pg.locator('#screen [data-rune-shop-buy]').count()
            check('ルーンの粉ショップ: 開いている採掘段階ぶんの未鑑定ルーン(TierⅠ・Ⅱ)', rows == await pg.evaluate("() => runeShopTier()") and rows >= 1, rows)
            await pg.screenshot(path=str(OUT / 'rune_shop.png'))
            await pg.evaluate("() => { STATE.runes = STATE.runes.slice(0, 20); saveState(); document.querySelector('[data-rune-shop-buy]').click(); document.querySelector('[data-rune-shop-buy]').click(); }"); await pg.wait_for_timeout(200)
            check('交換すると未鑑定のルーンが増える', await pg.evaluate("() => STATE.runes.filter(r => r.sealed).length") == 2)
            await pg.evaluate("() => goto('runes', {nav:true})"); await pg.wait_for_timeout(200)
            check('ルーン一覧の先頭に未鑑定、「まとめて開封」ボタン', await pg.locator('#screen .rune-cell .rune-ic.sealed').count() == 2 and await pg.locator('[data-rune-open-all]').count() == 1)
            await pg.screenshot(path=str(OUT / 'rune_sealed.png'))
            await pg.evaluate("() => document.querySelector('#screen .rune-cell').click()"); await pg.wait_for_timeout(150)
            await pg.evaluate("() => document.querySelector('[data-rune-unseal]').click()"); await pg.wait_for_timeout(200)
            check('押して開封するとレア度と効果が出る', await pg.evaluate("() => STATE.runes.filter(r => r.sealed).length") == 1 and 'が出た' in await pg.inner_text('.rd-flash'))
            await pg.screenshot(path=str(OUT / 'rune_unsealed.png'))
            await pg.evaluate("() => document.querySelector('[data-close-rune-detail]').click()"); await pg.wait_for_timeout(100)
            await pg.evaluate("() => document.querySelector('[data-rune-open-all]').click()"); await pg.wait_for_timeout(150)
            check('まとめて開封', await pg.evaluate("() => STATE.runes.filter(r => r.sealed).length") == 0)
