...
async def main():
    with game_url() as url:
        async with async_playwright() as p:
            ...
            # 詳細 → 強化
            await pg.evaluate("() => document.querySelector('#screen .rune-cell').click()"); await pg.wait_for_timeout(200)
            check('ルーンを押すと詳細', await pg.locator('#modal-layer .rune-detail-modal').count() == 1)
            uid = await pg.evaluate("() => runeDetail.uid")
            for _ in range(3):
                await pg.evaluate("() => document.querySelector('[data-rune-enhance]').click()"); await pg.wait_for_timeout(120)
            check('強化3回で+3、サブ効果が増えるか伸びる', await pg.evaluate(f"() => runeByUid('{uid}').lv") == 3 and await pg.locator('.rd-flash').count() == 1)
            ...
            check('枠を押すとルーンを選ぶ画面', await pg.locator('#modal-layer .rune-picker-modal .rp-row').count() == 21)
