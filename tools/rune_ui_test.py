...
async def main():
    with game_url() as url:
        async with async_playwright() as p:
            ...
            uid = await pg.evaluate("() => { const b = addRune(makeRune(3, 4, 'mdef')); addRune(makeRune(3, 3, 'mdef')); addRune(makeRune(3, 0, 'mdef')); addRune(makeRune(3, 4, 'hp')); saveState(); render(); openRuneDetail(b.uid); return b.uid; }"); await pg.wait_for_timeout(200)
            ...
            check('材料の候補は同じ効果・同じTierの2つ', await pg.locator('#modal-layer .rf-cand').count() == 2)
            await pg.evaluate("() => document.querySelectorAll('#modal-layer .rf-cand').forEach(b => b.click())"); await pg.wait_for_timeout(150)
            await pg.evaluate("() => document.querySelectorAll('#modal-layer .rf-cand:not(.on)').forEach(b => b.click())"); await pg.wait_for_timeout(150)
            check('2つ選ぶと結果の見込み(TierⅣ・紫 = 虹4・金3・白0の平均を切り捨て)', await pg.locator('#modal-layer .rf-result').count() == 1 and 'TierⅣ・紫' in await pg.inner_text('#modal-layer .rf-result'), await pg.inner_text('#modal-layer .rf-result') if await pg.locator('#modal-layer .rf-result').count() else '')
            ...
            check('合成するとTierⅣになり材料が消える', await pg.evaluate(f"() => runeByUid('{uid}').tier") == 4 and await pg.evaluate("() => STATE.runes.filter(r => r.main === 'mdef' && r.tier === 3 && r.rarity !== 4).length") == 0 and await pg.locator('.rd-flash').count() == 1)
            ...
            check('枠を押すとルーンを選ぶ画面', await pg.locator('#modal-layer .rune-picker-modal .rp-row').count() == 23)
            ...
            check('ルーン採掘は6段階', await pg.locator('#screen .stage-card.dungeon').count() == 6)
