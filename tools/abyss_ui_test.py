...
async def main():
    with game_url() as url:
        async with async_playwright() as p:
            ...
            await pg.evaluate("() => { if(battleUI) goto('battle'); closeModal(); stageTab = 'abyss'; Object.assign(ensureAbyss(), { floor: 18, checkpoint: 15, snap: { m68: { hp: 0.2, sp: 0, alive: true } } }); render(); }"); await pg.wait_for_timeout(200)
            await pg.evaluate("() => document.querySelector('[data-abyss-giveup]').click()"); await pg.wait_for_timeout(200)
            warn = await pg.locator('.abyss-giveup').inner_text()
            check('諦めるを押すと、チェックポイントからやり直しになる警告が出る', 'チェックポイントからやり直し' in warn and '16階' in warn, warn[:80])
            await pg.screenshot(path=str(OUT / 'abyss_giveup.png'))
            await pg.evaluate("() => document.querySelector('.abyss-giveup #close-result').click()"); await pg.wait_for_timeout(150)
            check('やめるなら何も変わらない', await pg.evaluate("() => ensureAbyss().floor === 18 && !!ensureAbyss().snap"))
            await pg.evaluate("() => document.querySelector('[data-abyss-giveup]').click()"); await pg.wait_for_timeout(150)
            await pg.evaluate("() => document.querySelector('[data-abyss-giveup-ok]').click()"); await pg.wait_for_timeout(200)
            check('諦めると16階に戻り全回復', await pg.evaluate("() => ensureAbyss().floor === 16 && ensureAbyss().snap === null && document.querySelector('[data-start-stage=ab_16]') !== null && !document.querySelector('[data-abyss-giveup]')"))
            await pg.evaluate("() => { closeModal(); addItem('stamina_60', 1); addItem('idle_12', 1); bagTab = 'other'; renderBag(); }")
            await pg.wait_for_timeout(300)
            check('アイテム欄に「使えるアイテム」', await pg.locator('.bag-cell[data-item-detail="stamina_60"]').count() == 1 and await pg.locator('.bag-cell[data-item-detail="idle_12"]').count() == 1)
            await pg.screenshot(path=str(OUT / 'abyss_items.png'))
            s0 = await pg.evaluate('() => STATE.stamina')
            await pg.evaluate("() => document.querySelector('.bag-cell[data-item-detail=\"stamina_60\"]').click()"); await pg.wait_for_timeout(200)
            ...
            await pg.evaluate("() => document.querySelector('.bag-cell[data-item-detail=\"idle_12\"]').click()"); await pg.wait_for_timeout(200)
            await pg.evaluate("() => document.querySelector('[data-id-use]').click()"); await pg.wait_for_timeout(200)
